...
- **多条隧道一起运行**：分别命名、配置端口、查看地址和日志，支持单独或全部启动、停止。
- **留在托盘继续工作**：点击窗口关闭按钮会隐藏到托盘；选择「退出」才会停止全部隧道并退出程序。
- **保存常用 FRPS**：集中管理多台 FRPS 服务器，每条隧道可以选择自己的配置。Cloudflare 仍共用一个账户。
- **爱莉粉与点击波纹**：主题颜色、动画速度都能在设置里调整，也可以选择「关闭动画」。Windows 11 标题栏跟随主题使用的浅色背景。
- **简体中文 / English**：界面语言即时切换并记住选择，穿透内核的原始日志保留原文。
- **横向布局**：连接与状态并排放置，设置页平滑切换；小窗口仍可滚动访问内容。
...
支持本机监听端口检测、实时日志、复制外网地址、一键停止和重启。隧道配置会保存在本机，下次打开可继续使用，但不会自动启动连接。FRPS 配置中的 token 使用 Windows 用户级加密保存。

分享前请确认本地服务已有适当的访问保护。软件不会替你关闭系统安全防护。

## 测试情况

已在本机 Windows 11 检查设置页切换与返回、中英文切换、多隧道独立进程管理、FRPS 配置切换、托盘隐藏和退出清理。按钮波纹检查覆盖点击位置、圆角裁剪、键盘操作、连续点击、动画速度与关闭动画。EXE 已完成独立环境启动检查，内置穿透内核通过校验。

此前完成过 Cloudflare 临时隧道公网访问测试，本轮界面更新没有重跑公网连通测试。多隧道测试使用本地测试进程，没有据此宣称真实 FRPS 转发通过。Windows 10、Cloudflare 真实账户授权与固定域名操作、HTTPS 源站证书及长期稳定性仍待验证。

动画测试与设备范围见 [性能记录](docs/animation-performance.md)，不代表所有屏幕和驱动的实测结果。
...
主要文件：`tunnel_gui.pyw` 为入口，`qt_ui.py` 与 `qml/` 负责界面，`tunnel_manager.py` 管理多条隧道，`tunnel_core.py` 管理穿透进程，`profile_store.py` 保存配置，`tray_shell.py` 提供托盘操作，`cloudflare_account.py` 处理账户与固定域名操作。旧 Tk 界面保留作历史测试参考。
